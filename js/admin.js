          ...
          <div class="modal-content" style="max-width:600px;">
            ...
            <div style="border-top:1px solid var(--border-color); padding-top:1.5rem;">
              ...
              ${history.logs.length === 0 ? '<p style="font-size:0.875rem; color:var(--text-secondary);">No logs found.</p>' : `
                <div style="max-height:200px; overflow-y:auto; border:1px solid #f1f5f9; border-radius:8px; padding-right:4px;">
                  <table style="width:100%; border-collapse:collapse; font-size:0.75rem;">
                    <thead style="background:#f8fafc; border-bottom:1px solid #f1f5f9;">
                      <tr>
                        <th style="padding:0.6rem; text-align:left;">Date</th>
                        <th style="padding:0.6rem; text-align:left;">Clock In</th>
                        <th style="padding:0.6rem; text-align:left;">Clock Out</th>
                    ...
                    <tbody>
                      ${history.logs.map(log => `
                        <tr style="border-bottom:1px solid #f8fafc;">
                          <td style="padding:0.6rem; font-weight:600;">${new Date(log.clockIn).toLocaleDateString()}</td>
                          <td style="padding:0.6rem; color:var(--text-secondary);">${new Date(log.clockIn).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</td>
                          <td style="padding:0.6rem; color:var(--text-secondary);">${log.clockOut ? new Date(log.clockOut).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) : '<span style="color:var(--success-color); font-weight:700;">ACTIVE</span>'}</td>
                          <td style="padding:0.6rem; text-align:right; font-weight:700; color:var(--accent-color);">${log.totalHours || 0}h</td>
                        </tr>
                      `).join('')}
                    </tbody>
                  </table>
