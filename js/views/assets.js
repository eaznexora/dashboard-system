...
const AssetHub = {
    ...
    async promptNewFolder() {
        ...
        const modalHtml = `
            <div id="${modalId}" class="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm">
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 animate-in fade-in zoom-in duration-200">
                    <h3 class="font-bold text-xl text-gray-800 mb-2">New Folder</h3>
                    <p class="text-sm text-gray-400 mb-6 font-medium uppercase tracking-wider">Storage / New Directory</p>
                    <input type="text" id="${modalId}-input" class="w-full px-5 py-3.5 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all mb-8 font-semibold text-gray-700" placeholder="Untitled Folder" autofocus>
                    <div class="flex justify-end gap-3">
                        <button onclick="document.getElementById('${modalId}').remove()" class="px-6 py-3 font-bold text-gray-400 hover:text-gray-600">Cancel</button>
                        <button id="${modalId}-done" class="px-8 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 active:scale-95 transition-all">Done</button>
                    </div>
                </div>
            </div>
        `;
    ...
    showCardMenu(e, id, type) {
        ...
        // Custom simple context menu for renamed/trash
        const menuHtml = `
            <div id="card-context-menu" class="fixed z-[120] bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 w-48 animate-in fade-in slide-in-from-top-2" style="top:${e.clientY}px; left:${e.clientX}px;">
                <button onclick="AssetHub.renameItem('${id}', '${type}')" class="w-full text-left px-4 py-3 rounded-xl hover:bg-blue-50 hover:text-blue-700 flex items-center gap-3 font-semibold transition-colors">
                    <i class="ph ph-pencil-simple text-blue-500"></i> Rename
                </button>
                <button onclick="AssetHub.deleteItem('${id}', '${type}')" class="w-full text-left px-4 py-3 rounded-xl hover:bg-red-50 hover:text-red-700 flex items-center gap-3 font-semibold transition-colors">
                    <i class="ph ph-trash-simple text-red-500"></i> Move to Trash
                </button>
                <div class="h-[1px] bg-gray-50 my-1"></div>
                <button onclick="document.getElementById('card-context-menu').remove()" class="w-full text-left px-4 py-2 rounded-xl hover:bg-gray-50 text-gray-400 font-bold text-xs uppercase tracking-widest text-center mt-1">Close</button>
            </div>
        `;
    ...
    async renameItem(id, type) {
        ...
        const modalHtml = `
            <div id="${modalId}" class="fixed inset-0 z-[130] flex items-center justify-center bg-black/40 backdrop-blur-sm">
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 animate-in fade-in zoom-in duration-200">
                    <h3 class="font-bold text-xl text-gray-800 mb-2">Rename Item</h3>
                    <input type="text" id="${modalId}-input" class="w-full px-5 py-3.5 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all mb-8 font-semibold text-gray-700" value="" autofocus>
                    <div class="flex justify-end gap-3">
                        <button onclick="document.getElementById('${modalId}').remove()" class="px-6 py-3 font-bold text-gray-400 hover:text-gray-600">Cancel</button>
                        <button id="${modalId}-done" class="px-8 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all">Done</button>
                    </div>
                </div>
            </div>
        `;
